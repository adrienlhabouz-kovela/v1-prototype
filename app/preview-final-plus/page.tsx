...
// ─── CompliancePillsPlus — variante locale (Modif 4) ─────────────────────
// Sur mobile, la ligne 2 « souveraineté & opération » n'est plus masquée
// silencieusement : elle reste accessible via un <details> discret avec
// summary « + 5 marqueurs souveraineté ». Sur sm+ comportement inchangé
// (ligne 2 affichée d'emblée).
const compliancePlusCore = ["HDS certifié", "RGPD + CNIL conforme"];
const compliancePlusSupport = [
  "Société française",
...
function CompliancePillsPlus() {
  return (
    <div className="space-y-2.5">
      ...
      {/* Ligne 2 — mobile : <details> ; sm+ : affichée d'emblée comme avant */}
      <details className="group sm:hidden">
        ...
        <p className="mt-2 text-[10.5px] leading-relaxed tracking-tight text-charcoal/55">
          {compliancePlusSupport.map((label, i) => (
            <span key={label}>
...
export default function LandingPreviewFinalPlus() {
  return (
    <div className="bg-white pb-20 text-navy-900 md:pb-0">
          ...
          <div className="flex items-start px-5 pt-10 pb-7 sm:px-10 sm:pt-14 sm:pb-8 lg:px-10 lg:pt-20 lg:pb-8 xl:px-16 xl:pt-28 xl:pb-8">
            <div className="w-full max-w-[680px]">
              ...
              {/* Conformité réglementaire — intégrée au hero, juste sous
                  ...
                  Modif 4 : ligne 2 accessible sur mobile via <details>. */}
              <div className="mt-5">
                <CompliancePillsPlus />
              </div>
      ...
      {/* Sections E (baseline terrain) + Bande preuve terrain SUPPRIMÉES —
          fusionnées en data strip de §1 Constat (Modif 3) pour éliminer
          2 ruptures et renforcer la conclusion-quote. */}
      ...
      {/* Section Ibis — TIER FINAL B1 : passé de 3 cards (Fondateur,
          Chirurgiens, Produit) à 2 cards étoffées (Chirurgiens, Produit).
          Card Fondateur retirée — bloc fondateur séparé abandonné par
      ...
      <section id="modele" className="mx-auto max-w-6xl px-6 py-14 reveal-target">
        ...
        {/* MOBILE — TIER FINAL A1 : les 2 colonnes bénéfices vivent dans
            un accordéon unique replié par défaut, pour ne pas alourdir
            le scroll pricing. */}
